void DoAuger() {
  ...
  case AUGER_CURRENT_LOW:
    ...
    if ((millis() - auger_state_entered) > shutdown[ALARM_AUGER_LOW_CURRENT]){  //turn auger off if auger current low for 3 minutes
		TransitionAuger(AUGER_ALARM);
  ...
  case AUGER_FORWARD:
    ...
    if ((millis() - auger_state_entered) > shutdown[ALARM_AUGER_ON_LONG]){  //turn auger off if auger runs non-stop for too long, use auger_direction_entered???
		TransitionAuger(AUGER_ALARM);
